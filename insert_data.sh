#! /bin/bash

if [[ $1 == "test" ]]
then
  PSQL="psql --username=postgres --dbname=worldcuptest -t --no-align -c"
else
  PSQL="psql --username=freecodecamp --dbname=worldcup -t --no-align -c"
fi

# Do not change code above this line. Use the PSQL variable above to query your database.

# Clear tables
$PSQL "TRUNCATE TABLE games, teams"

# Function to get team ID from teams table based on team name
get_team_id() {
    team_name="$1"
    $PSQL "SELECT team_id FROM teams WHERE name = '$team_name'"
}

# Read contents of games.csv, skip the first line, and process each line
cat games.csv | tail -n +2 | while IFS=',' read -r year round winner opponent winner_goals opponent_goals; do
    # Insert winner team into teams table
    $PSQL "INSERT INTO teams (name) VALUES ('$winner') ON CONFLICT (name) DO NOTHING"
  
    # Insert opponent team into teams table
    $PSQL "INSERT INTO teams (name) VALUES ('$opponent') ON CONFLICT (name) DO NOTHING"
    
    # Get winner and opponent team IDs from teams table
    winner_id=$(get_team_id "$winner")
    opponent_id=$(get_team_id "$opponent")
    
    # Insert game into games table
    $PSQL "INSERT INTO games (year, round, winner_id, opponent_id, winner_goals, opponent_goals) VALUES ('$year', '$round', $winner_id, $opponent_id, '$winner_goals', '$opponent_goals')"
done
